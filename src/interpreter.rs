...
impl Interpreter {
    pub fn run_proc_name(
        ...
        if let Expr::Call(name, args) = expr {
            if name.starts_with(vec!["builtin".to_string()].as_slice()) {
                ...
                return (
                    Expr::Literal(LiteralType::Null, "".to_string()),
                    variables.clone(),
                );
            } else {
    ...
    pub fn run_code(
        ...
        for expr in code {
            if let Expr::Import(rel, path) = expr.clone() {
                ...
                self.run_proc_name(expr, variables.clone(), namespace.clone());
    ...
    pub fn resolve_math(
        &mut self,
        val: Expr,
        variables: HashMap<Vec<String>, Expr>,
        namespace: Vec<String>,
    ) -> (LiteralType, String) {
